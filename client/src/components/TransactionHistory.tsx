...
import { Database, FileText, Loader2, ArrowRight } from "lucide-react";
...
export function TransactionHistory() {
  ...
  return (
    <div className="space-y-4 mt-16 mb-24">
        ...
        <AnimatePresence>
          {receipts.map((receipt) => (
            <motion.div
              ...
              className="group p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800 hover:border-emerald-500/30 hover:bg-zinc-900 transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
            >
              <div className="flex items-start sm:items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-zinc-600 mt-1 flex items-center gap-2">
                    ...
                    <span>•</span>
                    <span>via {receipt.channel}</span>
                  </div>
                </div>
              </div>
              <div className="text-xs text-zinc-600 font-mono text-right shrink-0 bg-zinc-950/50 px-3 py-1.5 rounded-lg border border-zinc-800/50">
